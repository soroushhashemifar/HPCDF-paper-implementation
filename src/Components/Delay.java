...
public class Delay {
    ...
    public Delay clone(Method new_method){
        Delay del = new Delay(new_method);

        del.rho = this.rho.clone();
        del.rhop = this.rhop.clone();

        del.f = this.f.clone();
        del.fp = this.fp.clone();

        del.n = this.n.clone();
        del.np = this.np.clone();

        del.P0 = this.P0.clone();
        del.PQ = this.PQ.clone();

        del.P0p = this.P0p.clone();
        del.PQp = this.PQp.clone();
...
//            System.out.println("Debug Please! Scheme: " + method.scheme.type); // this is for debug
     ...
     * @param j the index of the fog node
     */
     double calcDeployDelay(int a, int j) {
