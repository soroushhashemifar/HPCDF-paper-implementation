...
public class Cost {
    ...
    static Double[] CLOUD_UNIT_PROC_COST; // C'^p_k   unit cost of process. at cloud ser. k (per million instructions)
    static double[] FOG_UNIT_PROC_COST; // C^P_j  unit cost of process. at fog node j (per million instructions)

    static Double[] CLOUD_UNIT_STOR_COST; // C'^S_k   unit cost of storage at cloud server k (per byte per second)
    static double[] FOG_UNIT_STOR_COST;  // C^S_j  unit cost of storage at fog node j (per byte per second)

    static Double[][] FOG_CLOUD_COMM_UNIT_COST; // unit cost of communication between fog nodes and cloud servers
    private static Double[][] FOG_FOG_COMM_UNIT_COST; // unit cost of communication among fog nodes
    static Double[] FOG_CONTROLLER_COMM_UNIT_COST; // unit cost of communication between fog nodes and FSC

    public static double[] SERVICE_PENALY; // p_a
    ...
    public Cost(int NUM_CLOUD_SERVERS, int NUM_FOG_NODES, int NUM_SERVICES) {
        ...
        ArrayFiller.fill1DArrayWithConstantNumber(CLOUD_UNIT_STOR_COST, 0.000000004d);
        ...
        ArrayFiller.fill1DArrayRandomlyInRange(SERVICE_PENALY, 100d, 200d); // 10-20 aggregated and optimal. 100-200 for threshold and tau
    }
     ...
     * `time`
     */
    static double costPC(double time, int k, int a, double[] L_P, double lambdap_in[][]) {
     ...
     * @param L_S storage size of service a, in bytes
     */
    static double costSC(double time, int k, int a, double[] L_S) {
        return CLOUD_UNIT_STOR_COST[k] * L_S[a] * time;
    }
     ...
     * routed from fog node j
     */
    public static double calcAverageCost(double time, int[][] x, int[][] xp, int[][] x_backup, double Vper[], double q[], double lambda_in[][], double lambdap_in[][], double lambda_out[][], double[] L_P, double[] L_S, int[][] h, Method method) {
...
//        }
        ...
        double costEnergy = energy * 0.000002 * Parameters.TAU;

        Double c = costPC + costPF + costSC + costSF + costCff + costCfc + costViol + costDep + costEnergy;
        totalCost += c;
        return c;
    }
}
